...
class normalDistributionPlot extends baseModal {
    constructor() {
        var config = {
            ...
            local(
                {
                #Generating the sequence of length 1000,the lower and upper bounds of the sequence are computed using the quantile function of the Normal distribution  (qnorm)  with p=0.0005 and p=0.9995 respectively 
                lowProbBound =0.0005
                upperProbBound=0.9995
                lowerbound = stats::qnorm(p=lowProbBound,mean={{selected.mean | safe}},sd={{selected.sd | safe}})
                upperbound = stats::qnorm(p=upperProbBound,mean={{selected.mean | safe}},sd={{selected.sd | safe}})
                ...
                if({{selected.a | safe}})
                {
                  RcmdrMisc::plotDistr(.x, stats::dnorm(.x,mean={{selected.mean | safe}},sd={{selected.sd | safe}}),cdf=FALSE,xlab="x",ylab="Density",main=paste("Normal Distribution:  Mean={{selected.mean | safe}},Standard deviation={{selected.sd | safe}}"),regions=regionslist,col=c('{{selected.reg1col | safe}}','{{selected.reg2col | safe}}'),legend.pos='{{selected.c | safe}}'  )
                }
                else
                {
                  RcmdrMisc::plotDistr(.x, stats::pnorm(.x,mean={{selected.mean | safe}},sd={{selected.sd | safe}}),cdf=TRUE,xlab="x",ylab="Cumulative Probability",main=paste("Normal Distribution:  Mean={{selected.mean | safe}},Standard deviation={{selected.sd | safe}}"),regions=regionslist,col=c('{{selected.reg1col | safe}}','{{selected.reg2col | safe}}'),legend.pos='{{selected.c | safe}}'  )
                }
