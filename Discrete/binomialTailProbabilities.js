...
var localization = {
    en: {
        title: "Binomial Tail Probabilities",
        navigation: "Binomial Tail Probabilities",
        varvals: "Enter variable valie(s) separated by a comma. Example: 3,0.5",
...
class binomialTailProbabilities extends baseModal {
    constructor() {
        var config = {
            id: "binomialTailProbabilities",
            label: localization.en.title,
            modalType: "one",
            RCode: `
                    local(
                        {
                        result = stats::pbinom(c({{selected.varvals | safe}}), size={{selected.size | safe}}, prob={{selected.prob | safe}}, lower.tail={{selected.a | safe}} )
                        print(result)
                        }
                        )                    
                `
...
module.exports.item = new binomialTailProbabilities().render()